...
public class ColaboradorBo {
	 ...
	 * 
	 * @param colab Objeto contendo o colaborador
	 * @throws ClassNotFoundException
	 * @throws DadoInvalidoException
	 * @throws SQLException
	...
	private void validar(Colaborador colab) throws ClassNotFoundException, SQLException, DadoInvalidoException {
		...
		if (colabDao.isCpfCadastrado(colab.getCpf()))
			throw new DadoInvalidoException("\nCPF já cadastrado!");
		...
		if (colabDao.isPisCadastrado(colab.getPis()))
			throw new DadoInvalidoException("\nPIS já cadastrado!");
	...
	/**
	 * Método que valida de acordo com as regras os dados de um colaborador para
	 * atualizar
	 * 
	 * @param colaborador
	 * @throws ClassNotFoundException
	 ...
	 * @throws DadoInvalidoException
	 */
	private void validarAtualizacao(Colaborador colab)
		...
		if (!colabDao.isMesmoCpf(colab.getCpf(), colab.getLogin().getMatricula())) {
			if (colabDao.isCpfCadastrado(colab.getCpf()))
				throw new DadoInvalidoException("\nCPF já cadastrado!");
		}
		...
		if (!colabDao.isMesmoPis(colab.getPis(), colab.getLogin().getMatricula())) {
			if (colabDao.isPisCadastrado(colab.getPis()))
				throw new DadoInvalidoException("\nPIS já cadastrado!");
		}
