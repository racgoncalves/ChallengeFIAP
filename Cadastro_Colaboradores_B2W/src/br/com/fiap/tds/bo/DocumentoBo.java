...
public class DocumentoBo {
	...
	/**
	 * Valida o documento e aciona a camada de Dao para enviá-lo
	 * 
	 ...
	 */
	public void enviar(Documento doc) throws ClassNotFoundException, DadoInvalidoException, SQLException,
			ItemNaoEncontradoException, ItemCadastradoException, AtualizacaoNaoRealizadaException {
	...
	private void validarDocumento(Documento doc) throws ClassNotFoundException, SQLException,
		...
		if (doc.getNome().equals("CERTIFICADO DE RESERVISTA")) {
			...
			Colaborador colab = colabDao.pesquisar(doc.getLogin().getMatricula());

			if (!colab.getSexo().equals("MASCULINO"))
				throw new DadoInvalidoException(
						"\nPara enviar o certificado de reservista o sexo precisa ser 'MASCULINO'!");

			if (!reservistaVal.isMaiorDezoito(colab.getDataNascimento()))
				throw new DadoInvalidoException(
